...
WbusInterface::WbusInterface(HardwareSerial &refSer) : serial(refSer) {
  init();
}
...
void WbusInterface::init() {
  ...
  // This code is for Arduino ATMEGA328 with single hardware serial port (pins 0
  // and 1) PORTD=digital pins 0 to 7
  ...
  // initialize serial communication at 2400 bits per second, 8 bit data, even
  // parity and 1 stop bit
  serial.begin(2400, SERIAL_8E1);

  // 250ms for timeouts
  serial.setTimeout(1000);
  // Empty all queues. BRK toggling may cause a false received byte (or more
  // than one who knows).
 ...
 */
unsigned char WbusInterface::checksum(unsigned char *buf, unsigned char len,
                                      unsigned char chk) {
  for (; len != 0; len--) {
    chk ^= *buf++;
  }
 ...
 */
int WbusInterface::send(uint8_t addr, uint8_t cmd, uint8_t *data, int len,
  ...
  if (buf[0] != addr || buf[1] != (len + len2 + 2) || buf[2] != cmd) {
    ...
    // buf[2]); CommunicationsErrorCount++;
    return -1;
  }
...
int WbusInterface::listen(const uint8_t *addr, uint8_t *cmd, uint8_t *data,
                          int *dlen) {
  uint8_t buf[3];
  uint8_t chksum = 0;
  int len = 0;
 ...
 * addr: source/destination address pair to be expected or returned in case of
 * host I/O cmd: if pointed location is zero, received client message, if not
 * send client message with command *cmd. data: buffer to either receive client
 * data, or sent its content if client (*cmd!=0).  If *data is !=0, then *data
 * amount of data bytes will  be skipped. dlen: out: length of data.
 */
int WbusInterface::recvAns(const uint8_t *addr, const uint8_t *cmd,
                           uint8_t *data, int *dlen, int skip) {
  uint8_t buf[3];
  uint8_t chksum;
  int len;
 ...
 */
int WbusInterface::io(uint8_t cmd, uint8_t *out, uint8_t *out2, int len2,
                      uint8_t *in, int *dlen, int skip) {
  int err, tries;
  int len;
  unsigned char addr;
  ...
  // Appears that there is a small 70ms delay on the real Thermotest software
  // between requests, mimick this here which helps data reliability...
  delay(30);
...
int WbusInterface::get_basic_info(HANDLE_BASIC_WBINFO i) {
  ...
  //  tmp = IDENT_DEV_NAME; len = 1; err = wbus_io( WBUS_CMD_IDENT, &tmp, NULL,
  //  0, (unsigned char*)i->dev_name, &len, 1); if (err) goto bail;
  ...
  // tmp = IDENT_DEV_ID;  len = 1; err = wbus_io( WBUS_CMD_IDENT, &tmp, NULL, 0,
  // i->dev_id, &len, 1); if (err) goto bail; tmp = IDENT_DOM_CU;  len = 1; err
  // = wbus_io( WBUS_CMD_IDENT, &tmp, NULL, 0, i->dom_cu, &len, 1); if (err)
  // goto bail; tmp = IDENT_DOM_HT;  len = 1; err = wbus_io( WBUS_CMD_IDENT,
  // &tmp, NULL, 0, i->dom_ht, &len, 1); if (err) goto bail; tmp = IDENT_CUSTID;
  // len = 1; err = wbus_io( WBUS_CMD_IDENT, &tmp, NULL, 0, i->customer_id,
  // &len, 1); if (err) goto bail; tmp = IDENT_SERIAL;  len = 1; err = wbus_io(
  // WBUS_CMD_IDENT, &tmp, NULL, 0, i->serial, &len, 1); if (err) goto bail;
...
int WbusInterface::get_fault(unsigned char ErrorNumber,
                             HANDLE_ERRINFO errorInfo) {
  len = 2;
  cmdbuf[0] = ERR_READ;
