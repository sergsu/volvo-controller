...
/* Dataset commands */
#define WBUS_CMD_DS_START                                             \
  0x47 /* 1 byte = 1 start bloc mode. Issued before WBUS_CMD_BAUD and \
          CMD_DS_LSB */
...
#define WBUS_CMD_DS_MSB \
  0x05 /* 1 byte: address MSB. No returned data. For either read or write. */
#define WBUS_CMD_DS_STOP 0x09 /* 0 byte: stop block mode. */
#define WBUS_CMD_BAUD                       \
  0x34 /* Set block mode baudrate. 2 bytes. \
          0x80 0x00 38400                   \
          0x40 0x00 19200                   \
          0x20 0x00  9600 */
/* After issuing WBUS_CMD_DS_START and WBUS_CMD_BAUD, the host sents 0x03 <addr>
   <checksum> and gets as response 0x03 <addr> <data> <checksum> */
...
#define WBUS_TS_EWR                                        \
  0x36 /* WRITE_EEPROM [address],[byte1],[byte2]           \
          Write two bytes to eeprom. Address, byte1, byte2 \
          as parameter. */

#define WBUS_TS_REGR                                     \
  0x40 /* discovered by pczepek, Response length 0 byte. \
...
#define CMD_X_VCAL                                                      \
  1 /* Calibrate voltage. 3 bytes. 1 byte 0, 2 bytes voltage in mV, big \
       endian. */
#define CMD_X_FCAL                                                             \
  2 /* Flame detector calibration. 3 bytes. 1 byte 0, 2 bytes mOhm big endian. \
     */
...
#define WBUS_CMD_TEST \
  0x45 /* <1 byte sub system id> <1 byte time in seconds> <2 bytes value> */
#define WBUS_CMD_QUERY 0x50 /* Read operational information registers */
#define WBUS_CMD_IDENT \
  0x51 /* Read device identification information registers */
...
/* Status flags. Bitmasks below. STAxy_desc means status "x", byte offset "y",
 * flag called 2desc" */
...
#define QUERY_OPINFO0                                                         \
  0x04 /* Fuel type, max heat time and factor for shortening ventilation time \
...
#define QUERY_SENSORS \
  0x05 /*!< Assorted sensors. 8 bytes. Byte offsets below. */
#define SEN_TEMP \
  0 /*!< Temperature with 50�C offset (20�C is represented by 70) */
#define SEN_VOLT 1 /*!< 2 bytes Spannung in mili Volt, big endian */
#define SEN_FD 3   /*!< 1 byte Flame detector flag */
#define SEN_HE                                                             \
  4 /*!< 2 byte, heating power, in percent or watts (semantic seems wrong, \
       heating energy?) */
...
#define WB_STATE_BOADR                                                 \
  0x02 /* Burn out ADR (has something to due with hazardous substances \
          transportation) */
...
#define FPW_R                                                             \
  0 /*!< 2 bytes: Current fuel prewarming PTC resistance in mili ohm, big \
       endian */
#define FPW_P                                                                  \
  2 /*!< 2 bytes: Currently applied fuel prewarming power in watts, big endian \
     */

/* 0x51 Command parameters */
#define IDENT_DEV_ID 0x01 /*!< Device ID Number */
#define IDENT_HWSW_VER                                                     \
  0x02 /*!< Hardware version (KW/Jahr), Software version, Software version \
          EEPROM, 6 bytes */
#define IDENT_DATA_SET 0x03 /*!< Data Set ID Number */
#define IDENT_DOM_CU \
  0x04 /*!< Control Unit Herstellungsdatum (Tag monat jahr je ein byte) */
#define IDENT_DOM_HT \
  0x05 /*!< Heizer Herstellungsdatum (Tag monat jahr je ein byte) */
#define IDENT_TSCODE 0x06 /*!< Telestart code */
#define IDENT_CUSTID                                                       \
  0x07 /*!< Customer ID Number (Die VW Teilenummer als string und noch ein \
          paar Nummern dran) + test sig */
#define IDENT_U0 0x08 /*!< ? */
#define IDENT_SERIAL \
  0x09 /*!< 5 bytes: Serial Number. 2 bytes Test signature. */
#define IDENT_WB_VER                                                         \
  0x0a /*!< W-BUS version. Antwort ergibt ein byte. Jedes nibble dieses byte \
          entspricht einer Zahl (Zahl1.Zahl2) */
#define IDENT_DEV_NAME \
  0x0b /*!< Device Name: Als character string zu interpretieren. */
#define IDENT_WB_CODE \
  0x0c /*!< W-BUS code. 7 bytes. This is sort of a capability bit field */
...
#define WB_CODE_ST                                                          \
  17 /* Spark transmitter. Implies no Glow plug and thus no resistive flame \
        detection */
...
#define WB_CODE_CAVR \
  21 /* Combustion air fan level is in RPM instead of percent */
...
#define WB_CODE_HEW                                                        \
  36 /* Heating energy is in watts (or if not set in percent and the value \
        field must be divided by 2 to get the percent value) */
...
#define WBUS_TEXT_NAC \
  14 /*!< Nozzle air compressor (not standart, POELI specific) */
